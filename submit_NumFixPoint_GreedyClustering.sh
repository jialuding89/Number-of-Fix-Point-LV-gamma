#!/bin/bash
# submit_NumFixPoint_GreedyClustering.sh
# ---------------------------------------------------------------------------
# Slurm array job: each task runs _2_2_NumFixPoint_fx_k_theta_GreedyClustering.py
# for one (k, theta) pair.
#
# Submit with:
#   sbatch submit_NumFixPoint_GreedyClustering.sh
# ---------------------------------------------------------------------------

#SBATCH --job-name=NumFP_GC
#SBATCH --output=logs/NumFP_GC_%A_%a.out
#SBATCH --error=logs/NumFP_GC_%A_%a.err
#SBATCH --array=0-19                           # adjust to (len(K_VALUES)-1)
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --mem=32G
#SBATCH --time=48:00:00
#SBATCH --partition=cpu
#SBATCH --mail-user=k23017508@kcl.ac.uk
#SBATCH --mail-type=END,FAIL

echo "===== Job ${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID} started on $(hostname) at $(date) ====="

# ---------------------------------------------------------------------------
# Environment setup
# ---------------------------------------------------------------------------
cd /scratch/users/k23017508/files || { echo "Directory not found!"; exit 1; }
echo "Working directory: $(pwd)"

module purge
module load anaconda3 2>/dev/null || module load python 2>/dev/null

source ~/miniforge3/etc/profile.d/conda.sh
conda activate lv

echo "Python being used: $(which python)"

python - <<EOF
import numpy, numba
print("Python environment OK")
EOF
if [ $? -ne 0 ]; then
    echo "Python environment broken. Exiting."
    exit 1
fi

# ---------------------------------------------------------------------------
# Parameter grid
# ---------------------------------------------------------------------------
K_VALUES=(0.1 0.42 0.74 1.05 1.37 1.68 2.0 2.32 2.63 2.95 \
          3.26 3.58 3.89 4.21 4.53 4.84 5.16 5.47 5.79 6.0)

THETA_VALUES=(0.16)

# Fixed simulation parameters
S=500
IT=100000
N_SIMS=10000          # Greedy clustering is fast — can handle large n_sims

# ---------------------------------------------------------------------------
# Resolve (k, theta) for this array task
# ---------------------------------------------------------------------------
N_K=${#K_VALUES[@]}
N_THETA=${#THETA_VALUES[@]}

K_IDX=$(( SLURM_ARRAY_TASK_ID % N_K ))
THETA_IDX=$(( SLURM_ARRAY_TASK_ID / N_K ))

K=${K_VALUES[$K_IDX]}
THETA=${THETA_VALUES[$THETA_IDX]}

echo "Task ${SLURM_ARRAY_TASK_ID}: k=${K}, theta=${THETA}, S=${S}, IT=${IT}, n_sims=${N_SIMS}"

# ---------------------------------------------------------------------------
# Run
# ---------------------------------------------------------------------------
mkdir -p logs
python _2_2_NumFixPoint_fx_k_theta_GreedyClustering.py "${K}" "${THETA}" "${S}" "${IT}" "${N_SIMS}"

echo "===== Job finished at $(date) ====="
