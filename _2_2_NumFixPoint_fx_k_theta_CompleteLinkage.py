"""
_2_2_NumFixPoint_fx_k_theta_CompleteLinkage.py
-----------------------------------------------
Run a single simulation for one specific (k, theta) pair using Complete
Linkage hierarchical clustering to count distinct fixed-point attractors.
...
Output filename convention:
    result_CL_k<k>_theta<theta>_S<N_species>_IT<iterations>_nS<n_sims>.npz

Note: Complete Linkage requires O(n_sims^2) memory and time.
      Keep n_sims <= 5000 to avoid memory issues on the cluster.
"""
...
# ---------------------------------------------------------------------------
if len(sys.argv) < 3:
    print("Usage: python _2_2_NumFixPoint_fx_k_theta_CompleteLinkage.py <k> <theta> [S] [IT] [n_sims]")
    sys.exit(1)
...
n_sims     = int(sys.argv[5])  if len(sys.argv) > 5 else 1_000

print(
    f"[CompleteLinkage] Running: k={k_val}, theta={theta}, "
...
output_filename = (
    f"result_CL_k{fmt(k_val)}_theta{fmt(theta)}_S{N_species}_IT{iterations}_nS{n_sims}.npz"
)
...
np.savez(
    ...
    method=np.array(["CompleteLinkage"]),
)
